...
-- Add gender field to profiles table for personalized greeting
ALTER TABLE public.profiles ADD COLUMN gender_preference text CHECK (gender_preference IN ('masculino', 'femenino'));

-- Add belt_level to techniques table
ALTER TABLE public.techniques ADD COLUMN belt_level text;
ALTER TABLE public.techniques ADD COLUMN is_outside_gokyo boolean DEFAULT false;

-- Add video_file_url to tactical_notes for uploaded videos
ALTER TABLE public.tactical_notes ADD COLUMN video_file_url text;

-- Create exercises table to track exercise history
